...
//file_put_contents($filePath,$onikobe,  FILE_APPEND | LOCK_EX);
  ...
  if (in_array($value, $open_words)){
        echo  'open'. PHP_EOL;
  } elseif (in_array($value, $close_words)){
        echo 'close'. PHP_EOL;
  } else {
    echo $string;
        }
...
$arr = array('result' => [array('name' => 'Eboshi','currently' => $eboshi),array('name' => 'St-Mary','currently' => $stmary),array('name' => 'Yamagata-Zao','currently' => $yamagatazao),array('name' => 'SpringValley','currently' => $springvalley),array('name' => 'Jungle Jungle','currently' => $jungle),array('name' => 'Onikobe','currently' => $onikobe)]);
